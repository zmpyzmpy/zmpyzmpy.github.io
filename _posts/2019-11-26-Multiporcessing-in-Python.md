...
## 3. Benchmarks
### 3.1 Benchmark on data pipeline
...
#### 3.3.1 Segmentation Fault

​	According to [this link](https://github.com/apache/arrow/issues/2511), we can reproduce the segmentation fault by the following procedure:
...
1. Create a partitioned dataset with the following code:
   ...
   pq.write_to_dataset(table, root_path='./example_dataset', partition_cols=['one', 'two'])
   ```

2. Create a Parquet file from a PyArrow Table created from the partitioned Parquet dataset:

   ```python
   import pyarrow.parquet as pq
   
   table = pq.ParquetDataset('./example_dataset').read()
   pq.write_table(table, './example.parquet')
   ```

3. Run the above code in Jupyter Notebook would result in kernel die (due to segmentation fault):

   ![image-20191128173245077]({{site.url}}/images/posts/multi-image/4.png)
...
- Ray
  ...
  res = ray.get([run_res.remote() for i in range(4)])
  ```

  No error appeared for the above code using Ray.
...
- Multiprocessing, Pathos
  ...
  pool = Pool(4)
  pool.starmap(run_res, [_,_,_,_])
  ```

  The above code hangs forever...
...
- Futures
  ...
  r = executor.map(run_res, [(i) for i in range(4)])
  executor.shutdown()
  ```

  No error appeared for the above code using futures.
...
- Joblib
  ...
  pool = Parallel(4)
  pool(delayed(run_res)(i) for i in range(4))
  ```

  ![image-20191128173543027]({{site.url}}/images/posts/multi-image/5.png)

  An segmentation fault would be triggered when using Joblib
...
- Ray
  ...
  res = ray.get([foo.remote(i) for i in range(4)])
  ```
  ...
  ```python
     ...
     def foo(x):
         ...
         if x >= 3:
             ...
             sys.exit(1)
     with Pool(4) as pool:
         res = pool.map(foo, [1, 2, 3, 4])
     print(res)
  ```

  The above code would hang forever...
...
- Futures
  ...
  ```python
     ...
     with ProcessPoolExecutor(4) as pool:
         for res in pool.map(foo, [1, 2, 3, 4]):
             print(res)
  ```
...
- Joblib
  ...
  pool = Parallel(4)
  pool(delayed(foo)(i) for i in range(4))
  ```
...
| Package         | no pickle issue | no seg fault | no sys exit hang |
| --------------- | --------------- | ------------ | ---------------- |
| Ray             | ✔️               | ✔️            | ✔️                |
| Futures         | ❌               | ✔️            | ✔️                |
| Multiprocessing | ❌               | ❌            | ❌                |
...
> [https://towardsdatascience.com/modern-parallel-and-distributed-python-a-quick-tutorial-on-ray-99f8d70369b8](https://towardsdatascience.com/modern-parallel-and-distributed-python-a-quick-tutorial-on-ray-99f8d70369b8)

> [https://towardsdatascience.com/10x-faster-parallel-python-without-python-multiprocessing-e5017c93cce1](https://towardsdatascience.com/10x-faster-parallel-python-without-python-multiprocessing-e5017c93cce1)

> [https://yuanjiang.space/pathos-solves-PicklingError-for-multiprocessing](https://yuanjiang.space/pathos-solves-PicklingError-for-multiprocessing)

> [https://github.com/apache/arrow/issues/2511](https://github.com/apache/arrow/issues/2511)